					...
					if ( $v[0] == 1 ) {
						...
						switch ( $k ) {
							case 'twitter' :
								...
								$api_text = apply_filters( 'juiz_sps_share_text_for_' . $k, __( 'Share this article on Twitter', 'juiz-social-post-sharer' ) );
								break;

							case 'facebook' :
								$api_link = 'https://www.facebook.com/sharer/sharer.php?u=' . $url;
								$api_text = apply_filters( 'juiz_sps_share_text_for_' . $k, __( 'Share this article on Facebook', 'juiz-social-post-sharer' ) );
								break;

							case 'google' :
								$api_link = 'https://plus.google.com/share?url=' . $url;
								$api_text = apply_filters( 'juiz_sps_share_text_for_' . $k, __( 'Share this article on Google+', 'juiz-social-post-sharer' ) );
								break;
							...
							case 'viadeo' :
								...
								$api_text = __( 'Share this article on Viadeo', 'juiz-social-post-sharer' );
								break;

							case 'linkedin':
								$api_link = 'https://www.linkedin.com/shareArticle?mini=true&amp;ro=true&amp;trk=JuizSocialPostSharer&amp;title=' . $text . '&amp;url=' . $url;
								$api_text = apply_filters( 'juiz_sps_share_text_for_' . $k, __( 'Share this article on LinkedIn', 'juiz-social-post-sharer' ) );
								break;

							case 'digg':
								$api_link = 'https://digg.com/submit?phase=2%20&amp;url=' . $url . '&amp;title=' . $text;
								$api_text = apply_filters( 'juiz_sps_share_text_for_' . $k, __( 'Share this article on Digg' , 'juiz-social-post-sharer' ) );
								break;

							case 'stumbleupon':
								$api_link = 'https://www.stumbleupon.com/badge/?url=' . $url;
								$api_text = apply_filters( 'juiz_sps_share_text_for_' . $k, __( 'Share this article on StumbleUpon', 'juiz-social-post-sharer') );
								break;
							...
							case 'weibo':
        						...
        						$api_text = apply_filters( 'juiz_sps_share_text_for_' . $k, __( 'Share this article on Weibo', 'juiz-social-post-sharer' ) );
        						break;

							case 'vk':
								$api_link = 'https://vkontakte.ru/share.php?url=' . $url;
								$api_text = apply_filters( 'juiz_sps_share_text_for_' . $k, __( 'Share this article on VKontakte', 'juiz-social-post-sharer' ) );
								break;
				...
				if ( isset( $juiz_sps_options['juiz_sps_counter_option'] ) ) {
					if ( $juiz_sps_options['juiz_sps_counter_option'] == 'both' || $juiz_sps_options['juiz_sps_counter_option'] == 'total' ) {
						$juiz_sps_content .= ( 
											 ...
											 ?
											 '<' . $li . ' class="juiz_sps_item juiz_sps_totalcount_item"><span class="juiz_sps_totalcount" title="' . __( 'Total: ', 'juiz-social-post-sharer' ) . '"><span class="juiz_sps_t_nb"></span></span></' . $li . '>'
											 : '';
					}
				}
				$juiz_sps_content .= '</' . $ul . '>' . "\n\t";
